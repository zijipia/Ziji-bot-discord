...
module.exports.execute = async (interaction, lang) => {
  ...
  if (targetMessage.content) {
    const content = await translate(targetMessage.content, {
      to: lang?.name || 'en',
    });
  ...
  if (targetMessage.embeds.length) {
    ...
    if (revembed?.thumbnail?.url) embed.setThumbnail(revembed?.thumbnail?.url);
    if (revembed?.image?.url) embed.setImage(revembed?.image?.url);
    if (revembed?.color) embed.setColor(revembed?.color);

    if (revembed?.description) {
      const descriptions = await translate(revembed.description, {
        to: lang?.name || 'en',
      });
      embed.setDescription(descriptions.text);
    }
    if (revembed?.title) {
      const titles = await translate(revembed.title, {
        to: lang?.name || 'en',
      });
    ...
    if (revembed?.fields?.length) {
      for (const field of revembed.fields) {
        const fieldname = await translate(field.name, {
          to: lang?.name || 'en',
        });
