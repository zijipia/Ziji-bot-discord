...
module.exports.execute = async (interaction, query, lang) => {
  ...
  if (!voiceChannel) {
    return interaction.reply({
      content:
        lang?.Search?.NOvoiceChannel ?? 'Bạn chưa tham gia vào kênh thoại',
    });
  }
  const voiceMe = interaction.guild.members.cache.get(
    interaction.client.user.id,
  ).voice.channel;
  if (voiceMe && voiceMe.id !== voiceChannel.id) {
    return interaction.reply({
      content:
        lang?.Search?.NOvoiceChannel ?? 'Bot đã tham gia một kênh thoại khác',
    });
  }
  ...
  if (validURL(query)) {
    try {
      ...
      await player.play(voiceChannel, res, {
        nodeOptions: {
          ...
          metadata: queue?.metadata ?? {
            ...
            LockStatus: false,
            mess:
              interaction?.customId !== 'player_SelectionSearch'
      ...
      console.error(e);
      const response = {
        content: lang?.Search?.NOres ?? '❌ | Không tìm thấy bài hát',
        ephemeral: true,
      };
  ...
  const embed = new EmbedBuilder()
    ...
    .addFields(
      tracks.map((track, i) => ({
        name: `${i + 1}: ${track.title.slice(0, 50)} \`[${
          track.duration
        }]\``.slice(0, 99),
        value: ` `,
        inline: false,
