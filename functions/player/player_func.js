...
const getRelatedTracks = async (track, history) => {
  try {
    let tracks =
      (await track?.extractor?.getRelatedTracks(track, history))?.tracks || [];
...
module.exports = {
  ...
  execute: async (client, queue, tracks) => {
    const track =
      tracks ?? queue?.currentTrack ?? queue?.history?.previousTrack;
    ...
    const trackDuration =
      timestamps?.progress === 'Infinity' ? '∞' : timestamps?.progress;
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(
        `Volume: **${
          queue.node.volume
        }** % - Playing:  **${trackDuration}**${trackDurationSymbol} <a:_:${
          ZiIcons.animatedIcons[
            Math.floor(Math.random() * ZiIcons.animatedIcons.length)
          ]
        }>`,
      )
      .setColor('Random')
    ...
    const filteredTracks = relatedTracks
    ...
    const trackOptions = filteredTracks.map((track, i) => {
      return new StringSelectMenuOptionBuilder()
        ...
        .setDescription(
          `Duration: ${track.duration} source: ${track.queryType}`,
        )
        .setValue(`${track.url}`)
    ...
    const relatedTracksRow = new ActionRowBuilder().addComponents(
      new StringSelectMenuBuilder()
        ...
        .setPlaceholder(
          lang?.playerFunc?.RowRel ?? '▶ | Select a song to add to the queue',
        )
    ...
    if (queue.node.isPlaying() || queue.node.isPaused() || !queue.isEmpty()) {
      ...
      const functions = [
        ...
        {
          Label: !queue.metadata.LockStatus
            ? lang?.playerFunc?.Fields?.Lock || 'Lock'
            : lang?.playerFunc?.Fields?.UnLock || 'UnLock',
          Description: !queue.metadata.LockStatus
      ...
      const filteredFunctions = functions.filter((f) => {
        if (queue.isEmpty() && (f.Label === 'Shuffle' || f.Label === 'Queue'))
          return false;
      ...
      const functionRow = new ActionRowBuilder().addComponents(
        new StringSelectMenuBuilder()
          ...
          .setPlaceholder(
            lang?.playerFunc?.RowFunc ??
              '▶ | Chọn một chức năng khác để điều khiển player',
          )
      ...
      const buttonRow = new ActionRowBuilder().addComponents(
        ...
        CreateButton({
          id: 'pause',
          emoji: `${
            queue.node.isPlaying() ? `${ZiIcons.pause}` : `${ZiIcons.play}`
          }`,
          disable: false,
        }),
      ...
      embed
        .setDescription(
          lang?.playerFunc?.QueueEmpty ||
            `❌ | Hàng đợi trống\n✅ | Bạn có thể thêm 1 số bài hát`,
        )
        .setColor('Red')
        .addFields({
          name: ' ',
          value: `𓊝 ┃ ﹏﹏﹏﹏﹏﹏﹏﹏﹏﹏﹏﹏﹏﹏﹏ ┃ 𓊝`,
        });
    ...
    if (queue.repeatMode !== 0) {
      embed.addFields({
        name: `${lang?.playerFunc?.Fields?.Loop || 'Lặp lại'}: ${
