...
class MusicSearchCard extends Builder {
  ...
  async renderDefaultPlayer({ index, avatar, displayName, time }) {
    ...
    return JSX.createElement(
      ...
      {
        className:
          'flex items-center bg-white/15 rounded-xl p-2 px-3 justify-between',
      },
      JSX.createElement(
        'div',
  ...
  async render() {
    ...
    this.width = 1000;
    this.height =
      40 + (players.length > 10 ? 80 : 100) * Math.ceil(players.length / 2);
    this.adjustCanvas();

    const playerGroupChunks = chunkArrayInGroups(
      players,
    ...
    const processedPlayerGroups = await Promise.all(
      playerGroupChunks.map(async (playerGroup) => {
        const renderedPlayers = await Promise.all(
          playerGroup.map((player) => this.renderDefaultPlayer(player)),
        );
